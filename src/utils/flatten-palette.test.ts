...
it("Should work for default Tailwind colour palette", () => {
  const flattened = flattenPalette(options, require("tailwindcss/colors"));
...
it("Should ignore colours of ignored keys, if set", () => {
  ...
  expect(flattened).toStrictEqual({ "red-50": ["red", "50"] });
});
...
it("Should handle `undefined`", () => {
  const flattened = flattenPalette(options, undefined);
  expect(flattened).toStrictEqual({});
});
